...
export default function HomeFeedScreen() {
  ...
  const [showLocationFeed, setShowLocationFeed] = useState(false);
  ...
  const handleTouchEnd = (e: any, item: VideoPost) => {
    const swipeEndX = e.nativeEvent.pageX;
    const swipeDistance = swipeEndX - swipeStartX;
    
    // Swipe right detected
    if (swipeDistance > 50) {
      navigateToLocationFeed(item.location);
    }
  };

  const renderVideoPost = ({ item, index }: { item: VideoPost; index: number }) => (
    <TouchableWithoutFeedback
      onPressIn={handleTouchStart}
      onPressOut={(e) => handleTouchEnd(e, item)}
    >
      <View style={styles.videoContainer}>
        ...
        {/* Content Overlay - MUCH LOWER NOW */}
        <View style={styles.contentOverlay}>
          ...
          <View style={styles.postInfo}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => navigateToLocationFeed(item.location)}
              activeOpacity={0.7}
            >
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Location Feed Modal */}
      <Modal
        visible={showLocationFeed}
        animationType="slide"
        presentationStyle="fullScreen"
      >
        <View style={styles.locationFeedContainer}>
          <View style={styles.locationFeedHeader}>
            <TouchableOpacity onPress={() => setShowLocationFeed(false)}>
              <Ionicons name="arrow-back" size={28} color="white" />
            </TouchableOpacity>
            <View style={styles.locationFeedTitleContainer}>
              <Ionicons name="location-sharp" size={20} color="white" />
              <Text style={styles.locationFeedTitle}>{selectedLocation?.name}</Text>
            </View>
            <TouchableOpacity>
          ...
          <FlatList
            data={locationPosts[selectedLocation?.id] || []}
            renderItem={renderVideoPost}
            keyExtractor={(item) => item.id}
            pagingEnabled
            showsVerticalScrollIndicator={false}
...
const styles = StyleSheet.create({
  ...
  contentOverlay: {
    position: 'absolute',
    bottom: 20, // MUCH LOWER - just 20px from bottom
    left: 0,
    right: 0,
    flexDirection: 'row',
    paddingHorizontal: 16,
